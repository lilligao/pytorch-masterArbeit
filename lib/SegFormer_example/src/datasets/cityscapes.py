...
class CityscapesDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Disparity to Depth: https://github.com/mcordts/cityscapesScripts + https://github.com/mcordts/cityscapesScripts/issues/55
        disparity = cv2.imread(self.depths[index], cv2.IMREAD_UNCHANGED).astype(np.float32)
        disparity[disparity > 0] = (disparity[disparity > 0] - 1) / 256
        disparity = torch.tensor(disparity).unsqueeze(0)
        depth = torch.where(disparity > 0, 0.209313 * 2262.52 / disparity, 0)   # focal length * baseline / disparity

        if self.split == 'train':
            # Random Resize
            if config.USE_SCALING:
                random_scaler = RandResize(scale=(0.5, 2.0))
                image, depth, label = random_scaler(image.unsqueeze(0).float(), depth.unsqueeze(0).float(), label.unsqueeze(0).float())
                ...
                if image.shape[1] < 768 or image.shape[2] < 768:
                    ...
                    depth = F.pad(depth, border, 'constant', 0)
    ...
    def _load_image_and_label_paths(self):
        if self.split == 'train':
            ...
            depth_dir = os.path.join(self.root_dir, 'disparity/train')
            label_dir = os.path.join(self.root_dir, 'gtFine/train')
        elif self.split == 'val':
            image_dir = os.path.join(self.root_dir, 'leftImg8bit/val')
            depth_dir = os.path.join(self.root_dir, 'disparity/val')
        ...
        for city in os.listdir(image_dir):
            ...
            city_depth_dir = os.path.join(depth_dir, city)
            ...
            for image in os.listdir(city_image_dir):
                ...
                depth_path = os.path.join(city_depth_dir, image.replace('leftImg8bit', 'disparity'))
...
class RandResize(object):
    ...
    def __call__(self, image, depth, label):
        ...
        return image.squeeze(), depth.squeeze(0), label.squeeze(0).to(dtype=torch.int64)
