...
if __name__ == '__main__':
    ...
    results = []
    ############### for target image but merge output for each category######################
    ...
    # ############### for target image for each mask######################
    # for i in range(num_imgs): #num_imgs
    #     img, target = dataset[i]
    #     target_obj = target["labels_detection"]

    #     for i, id in enumerate(target_obj):
    #         #print(j)
    #         mask_visible = target["masks_visib"][i,:,:]==255
    #         #print("shape", mask_visible.shape)
    #         mask_visible = mask_visible.cpu()
    #         #scores = scores.cpu()
    #         fortran_mask = np.asfortranarray(mask_visible)
    #         rle = binary_mask_to_rle(fortran_mask)
